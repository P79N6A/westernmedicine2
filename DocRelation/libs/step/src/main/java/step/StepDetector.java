...
public class StepDetector implements SensorEventListener {
    ...
    private String TimeSeperateKey="STEP_TIME_SPACE";

    //每隔一个小时，就发送一次广播，通知外面，计步还在进行，外部可接受这个广播做一些持续的事情
    private String STEP_RUN_CONTINUE_BROAD_KEY="step_run_continue";
                    ...
                    if (direction == -mLastDirections[k]) {
                        ...
                        if (diff > mLimit) {
                            ...
                            if (isAlmostAsLargeAsPrevious && isPreviousLargeEnough && isNotContra) {
                                ...
                                //只要计步功能在运行,每隔一个小时发送一个广播，用于外部想要一直监听某个事件的需求
                                long timeSpace=System.currentTimeMillis() - (long)SPUtils.get(TimeSeperateKey,0L);
                                if(timeSpace> 1*60*60*1000){
                                    SPUtils.put(TimeSeperateKey,System.currentTimeMillis());
                                    Intent i = new Intent();
                                    i.setAction(STEP_RUN_CONTINUE_BROAD_KEY);
                                    StepManager.getmContext().sendBroadcast(i);
