...
/**
 * 自定义的textview，用来处理复制粘贴的消息  stone 可用
 *
 */
public class PasteEditText extends EditText {
    ...
    public boolean onTextContextMenuItem(int id) {
        if (id == android.R.id.paste) {
            ...
            if (text.startsWith(ChatFragment.COPY_IMAGE)) {
//                intent.setDataAndType(Uri.fromFile(new File("/sdcard/mn1.jpg")), "image/*");     
                text = text.replace(ChatFragment.COPY_IMAGE, "");
                ...
                ((Activity) context).startActivityForResult(intent, ChatFragment.REQUEST_CODE_COPY_AND_PASTE);
//                clip.setText("");
            }
        }
    ...
    protected void onTextChanged(CharSequence text, int start, int lengthBefore, int lengthAfter) {
        if (!TextUtils.isEmpty(text) && text.toString().startsWith(ChatFragment.COPY_IMAGE)) {
            setText("");
        }
