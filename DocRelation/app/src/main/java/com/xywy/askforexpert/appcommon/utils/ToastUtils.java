...
import com.xywy.uilibrary.BuildConfig;
...
public class ToastUtils {
    ...
    protected void debugToast(String string){
    ...
    private static void toastString(final String str, final int showTime) {
        h.post(new Runnable() {
            @Override
            public void run() {
                MyToast.getToast(YMApplication.getAppContext(), str, showTime).show();
            }
        });
    }
    ...
    private static void toastResId(int resId, int showTime) {
        toastString(YMApplication.getAppContext().getString(resId), showTime);
    }
