package com.xywy.askforexpert.model;

import java.io.Serializable;
import java.util.ArrayList;

/**
 * Created by jason on 2018/11/2.
 */

public class PatienTtitle implements Serializable{
    private String time;
    private ArrayList<PatienTtitle> patien_list;
    private String photo;
    private String id;
    private String uid;
    private String did;
    private String gid;
    private String patient_id;
    private String qid;
    private String username;
    private String doc_name;
    private String sex;
    private String age;
    private String mobile;
    private String sick;
    private String see_doc_time;
    private String advice;
    private String fz;
    private ArrayList<String> imgs;
    private String create_time;
    private String status;
    private String is_old;
    private int itemFlag;
    private boolean selected = false;

    public boolean isSelected() {
        return selected;
    }

    public void setSelected(boolean selected) {
        this.selected = selected;
    }

    public int getItemFlag() {
        return itemFlag;
    }

    public void setItemFlag(int itemFlag) {
        this.itemFlag = itemFlag;
    }

    public String getPhoto() {
        return photo;
    }

    public void setPhoto(String photo) {
        this.photo = photo;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getUid() {
        return uid;
    }

    public void setUid(String uid) {
        this.uid = uid;
    }

    public String getDid() {
        return did;
    }

    public void setDid(String did) {
        this.did = did;
    }

    public String getGid() {
        return gid;
    }

    public void setGid(String gid) {
        this.gid = gid;
    }

    public String getPatient_id() {
        return patient_id;
    }

    public void setPatient_id(String patient_id) {
        this.patient_id = patient_id;
    }

    public String getQid() {
        return qid;
    }

    public void setQid(String qid) {
        this.qid = qid;
    }

    public String getUsername() {
        return username;
    }

    public void setUsername(String username) {
        this.username = username;
    }

    public String getDoc_name() {
        return doc_name;
    }

    public void setDoc_name(String doc_name) {
        this.doc_name = doc_name;
    }

    public String getSex() {
        return sex;
    }

    public void setSex(String sex) {
        this.sex = sex;
    }

    public String getAge() {
        return age;
    }

    public void setAge(String age) {
        this.age = age;
    }

    public String getMobile() {
        return mobile;
    }

    public void setMobile(String mobile) {
        this.mobile = mobile;
    }

    public String getSick() {
        return sick;
    }

    public void setSick(String sick) {
        this.sick = sick;
    }

    public String getSee_doc_time() {
        return see_doc_time;
    }

    public void setSee_doc_time(String see_doc_time) {
        this.see_doc_time = see_doc_time;
    }

    public String getAdvice() {
        return advice;
    }

    public void setAdvice(String advice) {
        this.advice = advice;
    }

    public String getFz() {
        return fz;
    }

    public void setFz(String fz) {
        this.fz = fz;
    }

    public ArrayList<String> getImgs() {
        return imgs;
    }

    public void setImgs(ArrayList<String> imgs) {
        this.imgs = imgs;
    }

    public String getCreate_time() {
        return create_time;
    }

    public void setCreate_time(String create_time) {
        this.create_time = create_time;
    }

    public String getStatus() {
        return status;
    }

    public void setStatus(String status) {
        this.status = status;
    }

    public String getIs_old() {
        return is_old;
    }

    public void setIs_old(String is_old) {
        this.is_old = is_old;
    }

    public String getTime() {
        return time;
    }

    public void setTime(String time) {
        this.time = time;
    }

    public ArrayList<PatienTtitle> getPatien_list() {
        return patien_list;
    }

    public void setPatien_list(ArrayList<PatienTtitle> patien_list) {
        this.patien_list = patien_list;
    }
}
