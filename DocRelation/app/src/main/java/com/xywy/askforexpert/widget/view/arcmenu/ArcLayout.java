...
public class ArcLayout extends ViewGroup {
    ...
    public ArcLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
//        final int DEFAULT_CHILDSIZE = getContext().getResources().getDisplayMetrics().widthPixels/3;
        if (attrs != null) {
            ...
            mRadius=Math.max(a.getDimensionPixelSize(R.styleable.ArcLayout_radius, MIN_RADIUS), MIN_RADIUS);
            a.recycle();
        }
    }

//    private  int computeRadius(final float arcDegrees, final int childCount, final int childSize,
...
//                mRadius = computeRadius(Math.abs(mToDegrees - mFromDegrees), getChildCount(), mChildSize,
//                mChildPadding, MIN_RADIUS);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = 0; i < childCount; i++) {
            Rect frame = computeChildFrame(centerX, centerY, radius, degrees+ i * perDegrees, mChildSize);
    ...
    public void recenterArcView(){
        if (first){
            first=false;
            getViewTreeObserver().addOnDrawListener(new ViewTreeObserver.OnDrawListener() {
                @Override
                public void onDraw() {
                    float toXDelta= ViewCordinateUtil.getCenterGalobalX(centerView)-ViewCordinateUtil.getCenterGalobalX(ArcLayout.this);
                    float toYDelta=ViewCordinateUtil.getCenterGalobalY(centerView)-ViewCordinateUtil.getCenterGalobalY(ArcLayout.this);
//                    float toXDelta= ViewCordinateUtil.getGlobalVisible(centerView).centerX()-ViewCordinateUtil.getGlobalVisible(ArcLayout.this).centerX();
//                    float toYDelta=ViewCordinateUtil.getGlobalVisible(centerView).centerY()-ViewCordinateUtil.getGlobalVisible(ArcLayout.this).centerY();

                    setTranslationX(toXDelta);
                    setTranslationY(toYDelta);
    ...
    private void bindChildAnimation(final View child, final int index, final long duration) {
        ...
        Log.e("bindChildAnimation",index+":"+toXDelta+","+toYDelta);
//        Interpolator interpolator = mExpanded ? new AccelerateInterpolator() : new OvershootInterpolator(1.5f);
    ...
    private void onAllAnimationsEnd() {
        ...
        if (onExpandChangeListener!=null)
            onExpandChangeListener.onExpandChanged(mExpanded);
        if (!mExpanded){
