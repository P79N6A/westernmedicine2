package com.xywy.askforexpert.widget.module.im.imgroup;
...
public class PinyinIndexer extends View {
...
//	int  mTipFontColor = 0xff3399ff;
    ...
    public PinyinIndexer(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);
        initData(context, attrs);
    }

    public PinyinIndexer(Context context, AttributeSet attrs) {
        super(context, attrs);
        initData(context, attrs);
    }

    public PinyinIndexer(Context context) {
        super(context);
        initData(null, null);
    }
    ...
    private void initData(Context context, AttributeSet attrs) {
        ...
        if (context != null && attrs != null) {
            ...
            TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PinyinIndexer, 0, 0);

            mWidthOffset = a.getDimension(R.styleable.PinyinIndexer_widthOffset, mWidthOffset);
            mMinFontSize = a.getInteger(R.styleable.PinyinIndexer_minFontSize, mMinFontSize);
            mMaxFontSize = a.getInteger(R.styleable.PinyinIndexer_maxFontSize, mMaxFontSize);
            mTipFontSize = a.getInteger(R.styleable.PinyinIndexer_tipFontSize, mTipFontSize);
            mMaxBezierHeight = a.getDimension(R.styleable.PinyinIndexer_maxBezierHeight, mMaxBezierHeight);
            mMaxBezierWidth = a.getDimension(R.styleable.PinyinIndexer_maxBezierWidth, mMaxBezierWidth);
            mMaxBezierLines = a.getInteger(R.styleable.PinyinIndexer_maxBezierLines, mMaxBezierLines);
            mAdditionalTipOffset = a.getDimension(R.styleable.PinyinIndexer_additionalTipOffset, mAdditionalTipOffset);
            mFontColor = a.getColor(R.styleable.PinyinIndexer_fontColor, mFontColor);
            mTipFontColor = a.getColor(R.styleable.PinyinIndexer_tipFontColor, mTipFontColor);
            a.recycle();
        }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < ConstChar.length; i++) {
            ...
            // 绘制字母
            drawTextInCenter(canvas, ConstChar[i], xPos + adjustXPosAnimation(i, yPos), yPos);
            ...
            if (i == mChooseIndex) {
                ...
                if (mAnimating || mHideAnimation) {
                    ...
                    pos = xPos + adjustXPosAnimation(i, yPos) - mAdditionalTipOffset;
...
//	    	   canvas.drawLine(0, yPos, width, yPos, mPaint);
    ...
    private int adjustFontSize(int i, float yPos) {
        ...
        // 根据水平方向偏移量计算出一个放大的字号
        float adjustX = Math.abs(adjustXPosAnimation(i, yPos));
     ...
     * @return
     */
    private float adjustXPosAnimation(int i, float yPos) {
        ...
        if (dis > -mMaxBezierWidth && dis < mMaxBezierWidth) {
            ...
            if (dis > mMaxBezierWidth / 4) {
                for (int i = mMaxBezierLines - 1; i > 0; i--) {
                    ...
                    if (isFloatEqual(dis, -mBezier1[i].y)) // 落在点上
                    {
                        return mBezier1[i].x;
                    }
            ...
            for (int i = 0; i < mMaxBezierLines - 1; i++) {
                ...
                if (isFloatEqual(dis, -mBezier2[i].y)) {
                    return mBezier2[i].x;
                }
