...
import com.xywy.askforexpert.module.message.healthrecord.utils.HealthRecordUtil;
...
public class TrendView extends View implements CustomScrollView.callBackScrollStop {
    ...
    List<Float> mIntegerIntegerHashMap = new ArrayList<Float>();
    /**
     * 画圆点画笔
     */
    ...
    private List firstPointArray = new ArrayList();
    private int dataSize = 0;
    private List<String> xAXisdates = new ArrayList<>();
    private List<String> xAXisSeconddates = new ArrayList<>();
     ...
     * @param plist
     */
    public TrendView(Context context, List<ArrayList<Float>> plist) {
        super(context);
        this.mContext = context;
        pointList.clear();
    ...
    public void setPointList(final List<ArrayList<Float>> pointL) {
        pointList.clear();
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < pointList.size(); i++) {
            ...
            drawLineAndLineText(canvas, pointList.get(i), i);
     ...
     * @param index  画第几条线
     */
    private void drawLineAndLineText(Canvas canvas, List<Float> point, int index) {
    ...
    private void drawAxisLine(Canvas canvas) {
        ...
        if (axisLineList != null && axisLineList.size() > 0) {
            ...
            for (int i = 0; i < axisLineList.size(); i++) {
                ...
                mPath.moveTo(previousX, previousY);
                mPath.lineTo(src_W / 2, previousY);
                canvas.drawPath(mPath, LinePaint);
                mPath.reset();
                ...
                ////画刻度线上的文字
                //if(Calibration ==null){
                //    Calibration = new Paint();
                //    Calibration.setTextSize(13);
                //    Calibration.setColor(Color.parseColor("#4BFFFFFF"));
                //}
                //String text="";
                //if(isShowIntegerText){
                //    text = axisLineList.get(i).intValue()+"";
                //}else{
                //    text = axisLineList.get(i)+"";
                //}
                //
                //canvas.drawText(text,src_W-dip2px(getContext(),paddingLeftRightSpace),previousY,Calibration);
            }
        }
    }
     ...
     */
    private void drawCircle(Canvas canvas, List<Float> point, int index) {
    ...
    //假如图标绘制多条线的时候，
    public void setShowWhichLine(List<Boolean> whichShow) {
        showWhichLine.clear();
