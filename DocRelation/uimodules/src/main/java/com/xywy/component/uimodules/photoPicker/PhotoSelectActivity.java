...
public class PhotoSelectActivity extends Activity implements GetPhotoTask.AsyncTaskCallback, AdapterView.OnItemClickListener, PhotoListAdapter.OnSelectListener {
    ...
    private Button mOK;
    private LinearLayout mAlbumPanel;
    ...
    private void initView() {
        ...
        mAlbumPanel = (LinearLayout) findViewById(R.id.pp_folder_panel);
        mBottomPanel = findViewById(R.id.pp_bottom_panel);
    ...
    private void changeState(PPSelectState state) {
        mState = state;
        switch (mState) {
            case PP_SELECT_STATE_ALBUM://弹出相册选择框
                mAlbumPanel.setVisibility(View.VISIBLE);
                mAlbumPanel.getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
                    @Override
                    public void onGlobalLayout() {
                        mAlbumPanel.getViewTreeObserver().removeGlobalOnLayoutListener(this);
                        panelHeight = mAlbumPanel.getHeight();
                    }
                });
                //TODO 不知道为啥onGlobalLayout 方法只执行一次 所以把属性动画移出来
                ObjectAnimator.ofFloat(mAlbumPanel, "translationY", panelHeight+mBottomPanel.getHeight(), 0).setDuration(200).start();
                break;
            case PP_SELECT_STATE_PHOTO://隐藏相册选择框
//                mAlbumPanel.setVisibility(View.GONE);
                ObjectAnimator.ofFloat(mAlbumPanel, "translationY", 0, panelHeight+mBottomPanel.getHeight()).setDuration(200).start();
                break;
            default:
                break;
        }
